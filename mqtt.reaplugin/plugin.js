...
var __mqttBundle = (() => {
  ...
  var HEATING_PUBLISH_INTERVAL_MS = 2e3;
  var IDLE_PUBLISH_INTERVAL_MS = 5e3;
  var UNIQUE_ID_KEY = "uniqueId";
  function telemetryPublishIntervalMs(rawState) {
  ...
  function normalizeConfig(raw, storedUniqueId) {
    ...
    return {
      ...
      config: {
        ...
        haDiscoveryPrefix: typeof raw.HaDiscoveryPrefix === "string" && raw.HaDiscoveryPrefix.trim() ? raw.HaDiscoveryPrefix.trim().replace(/\/+$/, "") : "homeassistant",
        haEntityNamePrefix: typeof raw.HaEntityNamePrefix === "string" ? raw.HaEntityNamePrefix : "DE1+ ",
  ...
  function buildStateMessage(input) {
    ...
    const steamDisabled = !Number.isFinite(shotSettings?.targetSteamTemp) || shotSettings.targetSteamTemp < 135;
...
`) && (e = e.replace(Kx, "%0A")), e.includes("\r") && (e = e.replace(Qx, "%0D")), e.includes("	") && (e = e.replace(Yx, "%09")), n.pathname = e, n;
  ...
  var CONNECTION_VERIFICATION_TIMEOUT_MS = 1e4;
  function createMqttBridge({ host, config, onCommand, log }) {
    ...
    function publishState(stateMessage, onPublished) {
      ...
      client.publish(stateTopic, JSON.stringify(stateMessage), { qos: 1, retain: true }, (error) => {
        if (!error && generation !== void 0 && verification?.generation === generation) {
          verification.published = true;
          maybeCompleteVerification(generation);
        } else if (error && generation !== void 0 && verification?.generation === generation) {
  ...
  function createLoopbackJsonStream({ host, path, onJson, onStatus, onOpen, onClose, log }) {
  ...
  var SENSOR_DEFINITIONS = [
    ...
    ["Steam Temperature", "steam_temp", "steam_heater_temperature", { device_class: "temperature", state_class: "measurement", unit_of_measurement: "\xB0C" }],
    ["Target Group Temperature", "target_group_temp", "target_group_temperature", { device_class: "temperature", state_class: "measurement", unit_of_measurement: "\xB0C" }],
    ["Target Mix Temperature", "target_mix_temp", "target_mix_temperature", { device_class: "temperature", state_class: "measurement", unit_of_measurement: "\xB0C" }],
    ...
    ["Target Hot Water Temperature", "target_hot_water_temp", "target_hot_water_temperature", { device_class: "temperature", unit_of_measurement: "\xB0C" }],
  ...
  function availability(config, kind = "machine") {
    const expression = kind === "online" ? "value_json.online" : kind === "scale" ? "value_json.online and value_json.scale_connected" : "value_json.online and value_json.de1_connected";
    return [{
  ...
  function common(config, metadata, name2, key, availabilityKind) {
    return {
      name: `${config.haEntityNamePrefix}${name2}`,
  ...
  function buildDiscoveryMessages(config, metadata = {}, profileOptions = []) {
    const messages = [
  ...
  function createPlugin(host) {
    ...
    function enqueuePublish(options = {}) {
      ...
      publishSequence = publishSequence.then(() => publishNow(options)).catch((error) => log(`publish failed: ${error?.message ?? error}`));
    ...
    function profileOptions() {
      return [...new Set(runtime.profiles.map((record) => record?.profile?.title).filter((title) => typeof title === "string" && title))];
    }
    ...
    function syncDiscovery() {
      if (!bridge?.connected) return;
      const messages = config.haAutoDiscoveryEnable ? buildDiscoveryMessages(config, runtime.metadata ?? {}, profileOptions()) : [];
    ...
    function startStreams() {
      ...
      addStream({
        ...
        onOpen: () => {
          runtime.shotStreamFirstFrame = true;
        },
        onJson: (frame) => {
      ...
      onEvent(event) {
        ...
        switch (event?.name) {
          case "stateUpdate":
            ...
            break;
          case "shotStored":
            onShotStored(event.payload).catch((error) => log(`shot processing failed: ${error?.message ?? error}`));
            break;
